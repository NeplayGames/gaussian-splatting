...
def train_command(source, model, method="baseline", seed=0, iterations=1000):
    ...
    return [sys.executable,"train.py","-s",str(source),"-m",str(model),"--disable_viewer","--quiet","--eval","--iterations",str(iterations),"--test_iterations","-1","--method",method,"--seed",str(seed)]
