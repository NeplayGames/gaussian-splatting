...
# All rights reserved.
#
# Extended with DropGaussian-style saliency-driven pruning
#
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations,
             checkpoint_iterations, checkpoint, debug_from, edge_name,
             saliency_name, args):

    if not SPARSE_ADAM_AVAILABLE and opt.optimizer_type == "sparse_adam":
        sys.exit("Trying to use sparse adam but it is not installed, please "
                 "install the correct rasterizer using pip install [3dgs_accel].")

    first_iter = 0
    cls = Edges.get_edge_processor(edge_name)
    saliency_cls = Saliency.get_saliency_processor(saliency_name)
    combiner = LossCombiner(cls, saliency_cls,
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # -------------------------------
        # Saliency-driven DropGaussian
        # -------------------------------
        base_drop_rate = getattr(opt, "dropgaussian_base_drop_rate", 0.1)
        use_saliency_dropout = getattr(opt, "use_saliency_dropgaussian", False)
        prune_every = getattr(opt, "dropgaussian_prune_every", 2000)

        local_saliency_mean = 0.0
        if use_saliency_dropout:
            try:
                sal_map = saliency_cls(viewpoint_cam.original_image)
                if not isinstance(sal_map, torch.Tensor):
                    sal_map = torch.from_numpy(sal_map).float()
                sal_map = sal_map.to("cuda")
                sal_map = (sal_map - sal_map.min()) / (sal_map.max() - sal_map.min() + 1e-8)
                local_saliency_mean = float(sal_map.mean().clamp(0, 1))
            except Exception:
                local_saliency_mean = 0.0

        drop_rate = base_drop_rate * (1.0 - local_saliency_mean)
        drop_rate = float(max(0.0, min(0.95, drop_rate)))

        applied_dropout, original_opacity = False, None
        if use_saliency_dropout and drop_rate > 0.0 and iteration < opt.iterations:
            try:
                if hasattr(gaussians, "opacity"):
                    original_opacity = gaussians.opacity.data.clone()
                    comp = torch.ones_like(original_opacity[:, 0] if original_opacity.ndim == 2 else original_opacity)
                    drop = torch.nn.Dropout(p=drop_rate)
                    comp_after = drop(comp)
                    if original_opacity.ndim == 2:
                        gaussians.opacity.data *= comp_after[:, None]
                    else:
                        gaussians.opacity.data *= comp_after
                    applied_dropout = True
            except Exception:
                applied_dropout = False
        ...
        if applied_dropout and original_opacity is not None:
            gaussians.opacity.data = original_opacity
        ...
        edge_saliency_factor = combiner.compute_factor(image, gt_image)
        Ll1 = l1_loss(image, gt_image) * edge_saliency_factor
        ...
        with torch.no_grad():
            ...
            # Extra saliency-based pruning every N iterations
            if use_saliency_dropout and iteration % prune_every == 0 and iteration > opt.densify_from_iter:
                print(f"[ITER {iteration}] Saliency-based pruning...")
                try:
                    # Drop permanently very transparent blobs
                    low_opacity_mask = (gaussians.opacity.data.squeeze() < 1e-3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--edge_name", type=str, default="sobel",
                        choices=["sobel"])
    parser.add_argument("--saliency_name", type=str, default="Itti",
                        choices=["Boolean", "Itti"])

    # DropGaussian extensions
    parser.add_argument("--use_saliency_dropgaussian", action="store_true",
                        help="Enable saliency-weighted DropGaussian dropout + pruning")
    parser.add_argument("--dropgaussian_base_drop_rate", type=float, default=0.2,
                        help="Base dropout probability for DropGaussian")
    parser.add_argument("--dropgaussian_prune_every", type=int, default=500,
                        help="Iterations between permanent pruning of low-opacity Gaussians")
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args),
             ...
             args.debug_from, args.edge_name, args.saliency_name, args)
